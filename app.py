...
if not st.session_state.logado:
 ...
 senha_usuario = st.text_input("Digite aqui a senha para poder acessar o combinador de vídeos:", type="passwo
 st.markdown("""
 <style>
 .login-btn button {
 ...
 .login-btn button:hover { transform: translateY(-2px); box-shadow: 0 6px 20px rgba(0, 242, 254, 0.5) !im </style>
 """, unsafe_allow_html=True)
 ...
 st.markdown("<br><p style='text-align: center; color: #718096; font-size: 14px;'>Plataforma Comercial Protegelse:
 st.markdown("""
 <div style='display: flex; justify-content: space-between; align-items: center; padding: 20px; backgroun <div>
 <h2 style='margin:0; background: linear-gradient(45deg, #00F2FE, #4FACFE); -webkit-background-cl <p style='margin:0; color:#A0AEC0; font-size:14px;'>Acesso Premium Ativo • Produção de Criativos </div>
 </div>
 """, unsafe_allow_html=True)
 ...
 .sair-btn button:hover { background: #FE2C55 !important; color: white !important; border-color: #FE2 </style>
 """, unsafe_allow_html=True)
 ...
 desenvolvimentos = st.file_uploader("Arraste o conteúdo aqui (MP4)", accept_multiple_files=True, type=[" with col3:
 ...
 final_clip.write_videofile(nome_video_final, fps=24, codec="libx264", audio_codec="aac", log
 clip1.close()
 clip2.close()
 clip3.close()
 final_clip.close()
 ...
 barra_progresso.progress(progresso_atual, text=f"■ Criando variações... {idx+1}/{total_possi
