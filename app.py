...
background: rgba(20, 26, 43, 0.5); border-radius: 12px; margin-bottom: 25px; border: 1px solid rgba(0, 242,
254, 0.1);'>
 <div>
 <h2 style='margin:0; background: linear-gradient(45deg, #00F2FE, #4FACFE);
-webkit-background-clip: text; -webkit-text-fill-color: transparent; font-weight:800;'>■ Vibe Certa |
Combinação de Vídeos</h2>
...
Criativos TikTok & Kwai</p>
 ...
 .sair-btn button:hover { background: #FE2C55 !important; color: white !important; border-color:
#FE2C55 !important; }
 </style>
 """, unsafe_allow_html=True)
 ...
 with col1:
 st.markdown("<h3 style='color:#00F2FE !important;'>■ 1. Seção de Ganchos</h3>",
unsafe_allow_html=True)
 ganchos = st.file_uploader("Arraste os ganchos (MP4)", accept_multiple_files=True, type=["mp4"])
 with col2:
 st.markdown("<h3 style='color:#00F2FE !important;'>■ 2. Desenvolvimento</h3>",
unsafe_allow_html=True)
 desenvolvimentos = st.file_uploader("Arraste o conteúdo (MP4)", accept_multiple_files=True,
type=["mp4"])
 with col3:
 ...
 st.info(f"■ **Análise:** {total_possivel} combinações com áudio original.")
 ...
 f.write(f"file '{v2}'\n")
 f.write(f"file '{v3}'\n")
 comando = ["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", nome_lista_txt, "-c", "copy",
caminho_saida]
 ...
 st.markdown("</div>", unsafe_allow_html=True)
 if executar_lote:
 barra_progresso = st.progress(0, text="■■ Motores ativados...")
 try:
 ...
 barra_progresso.progress(progresso_atual, text=f"■ Criando variações...
{idx+1}/{total_possivel}")
 barra_progresso.progress(100, text="■ Compactando mídias...")
 ...
 st.download_button(label="■ BAIXAR LOTE COMPLETO (.ZIP)", data=file,
